...
if(isset($_POST['btnLogin'])){
	...
	if($loi!=""){
		...
		//$matkhau = md5($matkhau);
		//$result = mysqli_query($conn, "SELECT * FROM khachhang WHERE kh_tendangnhap ='" . $tendangnhap . "' AND kh_matkhau='" . $matkhau ."' AND kh_quantri= 1 ") or die(mysqli_error($conn));
        $result = mysqli_query($conn, "SELECT * FROM nguoidung nd JOIN vaitro vt ON nd.vt_id=vt.vt_id
		WHERE nd.nd_tendangnhap ='" . $tendangnhap . "' AND nd.nd_matkhau='" . $matkhau ."'  AND vt.vt_nguoidung=0") or die(mysqli_error($conn));
		if(mysqli_num_rows($result)==1){
			$row=mysqli_fetch_array($result, MYSQLI_ASSOC);
			$_SESSION["tendangnhap"]=$tendangnhap;
			$_SESSION["quantri"]=$row["vt_nguoidung"];
